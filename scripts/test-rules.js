...
async function main() {
  ...
  const day = async (date) => {
    const c = (await cal.loadContracts(orgId, ' AND c.id = ?', [contractId]))[0];
    const kids = (await cal.loadChildrenByContract(orgId, [contractId]))[contractId] || [];
    return cal.evaluateContractDay(c, date, kids, await cal.loadExceptions(orgId, date, date));
  };
