...
async function universalSearch(orgId, q, limit = 8) {
  ...
  const children = await all(`SELECT ch.id, ch.first_name, ch.last_name, ch.postcode, ch.address, ch.council_ref,
    ...
    WHERE ch.organisation_id = ? AND (LOWER(ch.first_name) LIKE ? ESCAPE '\\' OR LOWER(ch.last_name) LIKE ? ESCAPE '\\'
    ...
    ORDER BY ch.last_name LIMIT ?`, [orgId, p, p, p, p, p, p, limit]);
  ...
  const schools = await all(`SELECT id, name, postcode, address,
      (SELECT COUNT(*) FROM children WHERE school_id = schools.id AND organisation_id = schools.organisation_id AND status='active') AS child_count,
      (SELECT COUNT(*) FROM contracts WHERE school_id = schools.id AND organisation_id = schools.organisation_id AND status='active') AS contract_count
    FROM schools WHERE organisation_id = ? AND (LOWER(name) LIKE ? ESCAPE '\\' OR LOWER(postcode) LIKE ? ESCAPE '\\' OR LOWER(address) LIKE ? ESCAPE '\\') ORDER BY name LIMIT ?`, [orgId, p, p, p, limit]);
  ...
  const contracts = await all(`SELECT c.id, c.code, c.name, c.status, c.council_ref, s.name AS school_name, cl.name AS council_name,
    ...
    LEFT JOIN staff d ON d.id = c.driver_id
    WHERE c.organisation_id = ? AND (LOWER(c.code) LIKE ? ESCAPE '\\' OR LOWER(c.name) LIKE ? ESCAPE '\\' OR LOWER(c.council_ref) LIKE ? ESCAPE '\\' OR LOWER(c.route_info) LIKE ? ESCAPE '\\')
    ORDER BY c.code LIMIT ?`, [orgId, p, p, p, p, limit]);
  ...
  const staff = await all(`SELECT id, type, first_name, last_name, postcode, phone, status, badge_number,
      (SELECT COUNT(*) FROM contracts WHERE (driver_id = staff.id OR pa_id = staff.id) AND organisation_id = staff.organisation_id AND status='active') AS contract_count
    FROM staff
    WHERE organisation_id = ? AND (LOWER(first_name) LIKE ? ESCAPE '\\' OR LOWER(last_name) LIKE ? ESCAPE '\\' OR LOWER(first_name || ' ' || last_name) LIKE ? ESCAPE '\\'
       OR LOWER(postcode) LIKE ? ESCAPE '\\' OR LOWER(phone) LIKE ? ESCAPE '\\' OR LOWER(badge_number) LIKE ? ESCAPE '\\' OR LOWER(address) LIKE ? ESCAPE '\\')
    ORDER BY type, last_name LIMIT ?`, [orgId, p, p, p, p, p, p, p, limit * 2]);
  ...
  const councils = await all(`SELECT id, name, (SELECT COUNT(*) FROM contracts WHERE council_id = councils.id AND organisation_id = councils.organisation_id) AS contract_count FROM councils WHERE organisation_id = ? AND LOWER(name) LIKE ? ESCAPE '\\' ORDER BY name LIMIT ?`, [orgId, p, limit]);
  ...
  const vehicles = await all(`SELECT v.id, v.registration, v.make, v.model, v.seats, v.driver_id, s.first_name || ' ' || s.last_name AS driver_name FROM vehicles v LEFT JOIN staff s ON s.id = v.driver_id WHERE v.organisation_id = ? AND (LOWER(v.registration) LIKE ? ESCAPE '\\' OR LOWER(v.make) LIKE ? ESCAPE '\\' OR LOWER(v.model) LIKE ? ESCAPE '\\') LIMIT ?`, [orgId, p, p, p, limit]);
